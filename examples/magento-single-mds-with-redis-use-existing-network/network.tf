...
resource "oci_core_subnet" "redis_subnet_private" {
  cidr_block                 = cidrsubnet(var.vcn_cidr, 8, 3)
  display_name               = "redis_private_subnet"
  compartment_id             = var.compartment_ocid
  vcn_id                     = oci_core_virtual_network.magento_mds_vcn.id
  route_table_id             = oci_core_route_table.private_route_table.id
  security_list_ids          = [oci_core_security_list.private_security_list.id, oci_core_security_list.private_security_list_redis.id]
  dhcp_options_id            = oci_core_virtual_network.magento_mds_vcn.default_dhcp_options_id
